...
const ARCHETYPE_BACKGROUNDS: Record<ArchetypeId, string[]> = {
  ...
  physical_adept: [
    '{name} is Awakened without a tradition — magic channelled inward, no spells, no spirits. {flavor_cap}, and the results speak.',
    'The corp ran tests. {name} walked out. {flavor_cap} — a body attuned to magic, trained past what the charts cover.',
...
const PAST_PROFESSIONS: Record<ArchetypeId, string[]> = {
  ...
  physical_adept:     ['Street Fighter', 'Awakened Monk', 'Corporate Test Subject', 'Underground Duelist'],
...
const PERSONALITIES: Record<ArchetypeId, string[]> = {
  ...
  physical_adept:     ['precise about violence in ways that unsettle people', 'patient — built from the same material as stillness', 'does not explain what the magic is; just does the work', 'calm until the moment it ends'],
...
const MORAL_CODES: Record<ArchetypeId, string[]> = {
  ...
  physical_adept:     ['The body is a weapon. Treat it accordingly.', 'No chrome. Never. That\'s a line that doesn\'t move.', 'Magic is a responsibility — use it precisely or not at all.'],
...
const GOALS: Record<ArchetypeId, string[]> = {
  ...
  physical_adept:     ['Find out how far the training can actually go — no ceiling yet.', 'Protect someone the corps have decided is expendable.', 'Get clear of the Awakened community that wants to study them instead of letting them work.'],
...
const LOVES_HATES: Record<ArchetypeId, string[]> = {
  ...
  physical_adept:     ['Loves: a fight that uses everything. Hates: cyberware on principle — the shortcut that costs you.', 'Loves: the silence between training reps. Hates: anyone who thinks magic means easy.', 'Loves: open space to move. Hates: anyone who treats the Awakened as specimens.'],
